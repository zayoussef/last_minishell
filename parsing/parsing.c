...
/*   Updated: 2024/08/08 14:47:47 by yozainan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (((tokens[i].type == TOKEN_HERE_DOC && tokens[i + 1].type != TOKEN_WORD) || (tokens[i].type == TOKEN_REDIRECT_IN && tokens[i + 1].type != TOKEN_WORD) || (tokens[i].type == TOKEN_REDIRECT_OUT && tokens[i + 1].type != TOKEN_WORD) || (tokens[i].type == TOKEN_APPEND_OUT && tokens[i + 1].type != TOKEN_WORD)) && (tokens[i + 1].type != TOKEN_AMBIGUOUS))
	{
...
int check_syntaxe(Token *tokens, int nb_tokens)
{
	...
	while (i < nb_tokens && tokens[i].type != TOKEN_END)
	{
		if (tokens[i].type == NOT_DEFINE)
			return (1);
		if (tokens[i].type == TOKEN_OR || tokens[i].type == TOKEN_AND || tokens[i].type == TOKEN_LPR || tokens[i].type == TOKEN_BACKGROUND || tokens[i].type == TOKEN_RPR)
...
bool is_initial_token_error(Token *tokens, int i)
{
	if (i == 0 && (tokens[i].type == TOKEN_AND || tokens[i].type == TOKEN_OR || tokens[i].type == TOKEN_PIPE || tokens[i].type == TOKEN_BACKGROUND))
	{
		return (printf("Syntax error: unexpected token '%s'\n",
					   tokens[i].value),
				true);
...
bool is_final_token_error(Token *tokens, int i, int nb_tokens)
{
	if (i == nb_tokens - 1 && (tokens[i].type == TOKEN_AND || tokens[i].type == TOKEN_OR || tokens[i].type == TOKEN_PIPE || tokens[i].type == TOKEN_BACKGROUND))
	{
		return (printf("Syntax error: unexpected token '%s'\n",
					   tokens[i].value),
				true);
...
bool is_invalid_sequence(Token *tokens, int i)
{
	if ((tokens[i].type == TOKEN_AND && tokens[i + 1].type == TOKEN_AND) || (tokens[i].type == TOKEN_OR && tokens[i + 1].type == TOKEN_OR) || (tokens[i].type == TOKEN_BACKGROUND && tokens[i + 1].type == TOKEN_BACKGROUND))
	{
		return printf("Syntax error: unexpected token '%s'\n", tokens[i].value),
			   true;
	}
...
{
	return token.type == TOKEN_REDIRECT_IN || token.type == TOKEN_REDIRECT_OUT || token.type == TOKEN_APPEND_OUT || token.type == TOKEN_HERE_DOC || token.type == TOKEN_HERE_DOC_NO;
}

bool is_parenthesis(Token *tokens, int i)
{
	return (tokens[i].type == TOKEN_LPR && tokens[i + 1].type == TOKEN_WORD) || (tokens[i].type == TOKEN_RPR && tokens[i - 1].type == TOKEN_WORD && i > 1);
}
