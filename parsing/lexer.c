    ...
    while (**p != '\0' && (state->in_quotes || (!ft_isspace(**p) && **p != '|' && **p != '<' && **p != '>' && **p != '&' && **p != '(' && **p != ')')))
    {
        ...
        else if (**p == '$')
        {
            ...
            if (*p > start)
            {
                ...
                if (is_ambiguous(expanded_value) && *num_tokens > 0 && (tokens[*num_tokens - 1].type == TOKEN_REDIRECT_OUT || tokens[*num_tokens - 1].type == TOKEN_APPEND_OUT || tokens[*num_tokens - 1].type == TOKEN_REDIRECT_IN))
                {
                    free(expanded_value);
                    flag = 1;
                }
